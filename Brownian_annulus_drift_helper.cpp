...
double winding_angle(double T, double r0, double ra, double rb, double D,double C, double dt)
{
    double t = 0;
    mwSize n = 10000;
    double theta = 0;
...
{
	//X = Brownian_annulus_drift_helper(M,T,r0,ra,rb,D,C,dt)
    ...
    const double rb = mxGetScalar(prhs[4]);
    const double D =  mxGetScalar(prhs[5]);
    const double C =  mxGetScalar(prhs[6]);
    const double dt =  mxGetScalar(prhs[7]);

    const mwSize dim[2] = {1,M};
	plhs[0] = mxCreateNumericArray(2,dim,mxDOUBLE_CLASS,mxREAL);
	double *X = mxGetPr(plhs[0]);

	for(int i = 0; i < M; i++) X[i] = winding_angle(T,r0,ra,rb,D,C,dt);
