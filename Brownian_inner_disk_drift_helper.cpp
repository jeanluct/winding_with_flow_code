...
double winding_angle(double T, double r0, double ra, double D,double C, double dt_min)
{
    double t = 0;
    mwSize n = 10000;
    const double winding_per_step = .1/2/D;
    double theta = 0;
    double x0 = r0, x1, x2;
    double y0 = 0, y1, y2;
    double dx, dy;
    ...
    for(int m=0;t<T;m++)//&&m<=10
    {
        ...
        for (int i = 0; i < n-1 && t < T; i++)
        {
            double dt = dt_min + .1*min(1000.,winding_per_step*((x0*x0 + y0*y0)-ra*ra));//use non-uniform time step for computation speed
            ...
            if (x1*x1+y1*y1 < ra*ra)
            {
                double tt,nx,ny;
                dx = x1 - x0;
                dy = y1 - y0;

                tt = -(x0*dx+y0*dy + sqrt((x0*dx+y0*dy)*(x0*dx+y0*dy)-(dx*dx+dy*dy)*(x0*x0+y0*y0-ra*ra)))/(dx*dx + dy*dy);

                nx = x0 + tt*dx;
                ny = y0 + tt*dy;

                x2 = x1 - 2*(1-tt)*(dx*nx+dy*ny)/(nx*nx+ny*ny)*nx;
                y2 = y1 - 2*(1-tt)*(dx*nx+dy*ny)/(nx*nx+ny*ny)*ny;
